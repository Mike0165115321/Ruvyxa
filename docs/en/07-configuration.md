...
| ------------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Root          | `appDir`, `outDir`, `runtime`, `react`, `typedRoutes`, `typescript.strict`                                                       | Keep defaults unless the source/output layout or target requires a change. `runtime` is `node`, `bun`, `deno`, `edge`, or `static`; the CLI target can override it. `typedRoutes` also requires `.ruvyxa/types/**/*.d.ts` in the tsconfig `include`. |
