...
>
> **จะได้เรียนรู้:** 3 คำสั่งที่ใช้ทุกวัน + เครื่องมือ diagnostic ที่อธิบายแอปของคุณกลับมาให้ฟัง
> เมื่อมีอะไรไม่ชัดเจน

เพิ่งเริ่มต้น? ใช้แค่ 3 คำสั่งนี้ในชีวิตประจำวัน:

```bash
npm run dev     # เขียนโค้ด — server ในเครื่อง reload อัตโนมัติ
npm run build   # เตรียมขึ้น production
npm run start   # รัน production build ในเครื่อง
```

ที่เหลือในหน้านี้เป็นเครื่องมือ diagnostic หยิบใช้เมื่อต้องการ — ตัวที่ควรรู้จักถัดไปคือ `analyze`
กับ `check` เพราะจับปัญหาให้ก่อน deploy
...
# รองรับ node, bun, static, vercel, netlify, cloudflare, railway, render, firebase, aws
# หรือชื่อแพ็กเกจ adapter ใดก็ได้
# ถ้าไม่ระบุ adapter เลย ระบบ detect VERCEL/NETLIFY/CF_PAGES จาก build environment ให้อัตโนมัติ
